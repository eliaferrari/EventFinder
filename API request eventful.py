...
category = 'politics_activism' # update control category in database!! and control categories in API
pagenum = '100'
API_eventful = 'http://api.eventful.com/rest/events/search'

# API request
payload = {'app_key':'kxPnNrdf6DhPm93k','page_size':pagenum,'location':'Switzerland','date':'Future','category':category}
...
for event in events:
    ...
    connection.commit()
    file.write(title+','+ort+',0,'+ort+','+beschreibung+','+weblink+',1,'+category+',,'+geom+'\n')
file.close()